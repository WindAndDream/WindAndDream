...
let interArr: CustomInter[] = [
  { school: 'aaa', gender: '男性' }
]
 ...
 */
const stringArr: GenericArr<string[]> = [
  ['a', 'b', 'c']
]
 ...
 */
const genericArr: GenericArr<GenericArr<string>> = [
  ['啊哈', '哦耶', '曼波']
]
...
1. **原始值**
   ...
   我们首先要知道，`undefined` 和 `null` 都是特殊的原始值，不参与原型链。

   其他原始值，如：`string、number、boolean、symbol、bigint`，在调用属性或方法时，会被 **临时包装** 为对应的对象，原型链最终会走到 **null** 。
